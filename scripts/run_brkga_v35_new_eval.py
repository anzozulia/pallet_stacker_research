"""Run v3.5 NEW (wall fix + layer-build mode 3) on BR instances.

Same as run_brkga_v35_eval.py but writes to a different checkpoint file
so we can directly compare OLD vs NEW.
"""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="results/checkpoints/brkga_v35_new_results.json")
    args = ap.parse_args()

    cfg = geometric_only_config(max_pallets=1)
    print(f"v3.5 NEW: pop={args.pop}/{args.n_pops}pops, modes={args.n_modes}, "
          f"restarts={args.n_restarts}, LS={args.ls_budget}s, "
    ...
    print("\n=== v3.5 NEW Summary ===", file=sys.stderr)
    by_set = {}
    for r in results:
