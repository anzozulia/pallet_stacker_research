"""
ab_test_decoder_cstr_layer.py — Phase 4d A/B test:
Cython decode_layer_njit_cstr vs Numba reference.

Run inside Docker:
    docker run --rm -v "$(pwd)":/app pallet-packer:dev \
        python scripts/ab_test_decoder_cstr_layer.py
"""
...
def ab_one(rng, n_boxes, L, W, H, max_pallets,
    ...
    nb_bins = nb.decode_layer_njit_cstr(
        bps, n_rots, dims, L, W, H, max_pallets, po_nb,
        weights, mlot, rfs, weight_cap, support_ratio,
        require_centroid,
        cxmn, cxmx, cymn, cymx, cmlf, cact,
    )
    cy_bins = cy.decode_layer_njit_cstr(
        bps, n_rots, dims, L, W, H, max_pallets, po_cy,
        weights, mlot, rfs, weight_cap, support_ratio,
        require_centroid,
        cxmn, cxmx, cymn, cymx, cmlf, cact,
    )
...
def ab_test_suite(rng, n_cases=10):
    ...
    print(f"=== A/B correctness — decode_layer_njit_cstr ({len(cfgs)} configs) ===")
    fail = 0
    for c in cfgs:
        per_fail = 0
    ...
    if fail:
        ...
        sys.exit(1)
    total = len(cfgs) * n_cases
    print(f"All {total} cases bit-identical across {len(cfgs)} configs.")


def microbenchmark(rng):
    print("\n=== Microbenchmark (n=100, 500 repeats) ===")
    ...
    tail = (weights, mlot, rfs, 1e18, 0.5, 0,
            -1e18, 1e18, -1e18, 1e18, 0.0, 0)
    for _ in range(10):
        nb.decode_layer_njit_cstr(*args, po_nb, *tail)
        cy.decode_layer_njit_cstr(*args, po_cy, *tail)
    n_repeat = 500
    ...
    print(f"  decode_layer_njit_cstr: "
